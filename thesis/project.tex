...
\begin{document}
	\begin{center}
		...
		\textbf{Baltymų termostabilumo nusakymas 
		naudojant sekų reprezentacijas iš baltymų kalbos modelių}
	...
	\begin{flushright}
        ...
        Supervisor: Dr. Kliment Olechnovič 
    ...
    Although the classifier showed promising results, yet an
    ...
    These results contribute to the further development of
	...
	The transformer is a neural network model, which is made of encoder-decoder architecture 
	...
	result of using the script, a vector of 1280 dimensions for each protein is 
	generated.
	...
	1022 amino acids (two more amino acids are removed due to beginning and end 
	tokens needed for the encoding).
	...
	\href{https://bfd.mmseqs.com}{BFD-100} data set \cite{steinegger2019protein}, \cite{steinegger2018clustering}
	...
	predetermined limitation for the protein's size to generate its embedding. 
	...
	However, the analysis of other representations was carried 
	...
	The later change determined  insignificantly different prediction values compared 
	to the prediction values from the earlier work.

	As previously, the output of all models is a number from the interval 
	between 0 and 1 (both included). The binary class label is assigned 
	using the threshold of 0.5: if the prediction is greater than or 
	equal to 0.5, the protein is considered as thermostable (gets a label '1'),
	otherwise the protein's label is assigned to be '0'.
	...
	pairs of components (Figure \ref{figure:correlationComponentsMaxAndMean}) 
	shows that there are peaks 
	...
	statement that high maximum values are not dominating 
	because the mean of absolute mean correlation coefficients is very 
	small (close to zero) 
	between the pairs.
	...
	\begin{figure}[!h]
		...
		\includegraphics[scale=0.7]{ESM_PT_comparison_MCC_ROC_scores.png}
	...
	hidden layers achieved better results than the single-layer 
	models: the best model that used ProtTrans embeddings reached
	MCC of 0.916, meanwhile the SLP using ProtTrans embeddings 
	reached MCC of 0.901.

	\begin{figure}[h!]
		\centering
		\begin{tabular}{@{}c@{}}
			\includegraphics[scale=0.7]{MLP_ESM.png}
		\end{tabular}

		\begin{tabular}{@{}c@{}}
			\includegraphics[scale=0.7]{MLP_PT.png}
		\end{tabular}
	...
	octiles embeddings will be used. Although models that used 
	...
	\begin{figure}[h!]
		...
		\includegraphics[scale=0.7]{conclusions_plot.png}
    ...
    patikrinti, ar kitokie įvesties vektoriai suteikia
